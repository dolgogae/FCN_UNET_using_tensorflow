...
class segmentation_model():
    def __init__(self, model, input_shape, prertrained=False, base=4):
        self.model = model
        self.input_shape = input_shape
        self.base = base
        self.pretrained = prertrained

        if self.pretrained:
            path = os.path.join('models', model_name+'.model')
            if os.path.exists(path):
                model = load_model(path, custum_objects={'dice': dice})
                return model
            else:
                print('Falied to load existing model at: {}'.format(path))
    ...
    def fcn_8(self):
        ...
        model.compile(optimizer=Adam(1e-4), loss=self.loss, metrics=[dice])
        model.summary()
